...
struct BMP {
    ...
    void write_pixel(const uint32_t& x, const uint32_t& y, const uint8_t& B, const uint8_t& G, const uint8_t& R, const uint8_t& A) {
        data[bmp_info_header.bit_count / 8 * (y * bmp_info_header.width + x) + 0] = B;
        data[bmp_info_header.bit_count / 8 * (y * bmp_info_header.width + x) + 1] = G;
        data[bmp_info_header.bit_count / 8 * (y * bmp_info_header.width + x) + 2] = R;
        if (bmp_info_header.bit_count / 8 == 4) {
            data[bmp_info_header.bit_count / 8 * (y * bmp_info_header.width + x) + 3] = A;
        }
    }

    void write_pixel(const uint32_t& x, const uint32_t& y, const uint8_t& B, const uint8_t& G, const uint8_t& R, const uint8_t& A, const uint32_t& channels) {
    ...
    void fill_region(uint32_t x0, uint32_t y0, uint32_t w, uint32_t h, uint8_t B, uint8_t G, uint8_t R, uint8_t A) {
        ...
        for (uint32_t y = y0; y < y0 + h; ++y) {
            for (uint32_t x = x0; x < x0 + w; ++x) {
                write_pixel(x, y, B, G, R, A, channels);
            }
        }
    }
